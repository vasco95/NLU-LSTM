...
class Model(object):
    def __init__(self, datain, is_training,
                ...
                num_layers = 1, init_scale = 0.5, dropout = 0.1,
...
def test(test_data, vocabulary, batch_size, model_path,\
            word_unmap, step_len = 32, hidden_size = 512, num_layers = 1):
    # setup data and models
    ...
    with tf.Session(config = config) as sess:
        ...
        cnt = 0
        for batch in range(m.num_strides):
            # if batch % step_len == 0:
            #     current_state = np.zeros((num_layers, 2, m.batch_size, m.hidden_size))

            softmax_out, current_state, targets, accuracy, cost =\
                            sess.run([m.softmax_out, m.state, m.targets, m.accuracy, m.cost],
                            feed_dict = {m.init_state: current_state})
            prob = softmax_out[0][targets[0]]
            perplexity += np.log(prob)
            mlw = np.argmax(softmax_out)
            # print -np.log(prob), cost
...
#        num_layers = num_layers)
    ...
    #test(train_data, vocabulary, batch_size = 1,\
    #   model_path = data_path + model_save_name + '-17',
    #   step_len = step_len, hidden_size = hidden_size,\
    #   num_layers = num_layers, word_unmap = word_unmap)
    generate_text(word_map, word_unmap, vocabulary, batch_size = 1,\
        model_path = data_path + model_save_name + '-final',
        step_len = 30, hidden_size = hidden_size,\
        num_layers = num_layers)
